...
class VideoEditor:
    ...
    def cut_video(self, video_path, start_time, end_time):
        if not os.path.exists(video_path):
            ...
            return None
        try:
            logging.info(f"[CUT_VIDEO] Starting video cut: {video_path}")
            logging.info(f"[CUT_VIDEO] Cut times - Start: {start_time:.2f}s, End: {end_time:.2f}s, Duration: {end_time - start_time:.2f}s")
            ...
            if end_time > clip_duration:
                logging.warning(f"[CUT_VIDEO] End time {end_time:.2f}s exceeds video duration {clip_duration:.2f}s, adjusting to {clip_duration:.2f}s")
                end_time = clip_duration
            
            if start_time >= end_time:
                logging.error(f"[CUT_VIDEO] Invalid cut times: start ({start_time:.2f}s) >= end ({end_time:.2f}s)")
                clip.close()
                return None
            
            cut_clip = clip.subclip(start_time, end_time)
            logging.info(f"[CUT_VIDEO] Writing cut video to: {output_path}")
            cut_clip.write_videofile(output_path, logger=None)
            
            clip.close()
            logging.info(f"[CUT_VIDEO] Video cut successfully: {output_path}")
            return output_path
        except Exception as e:
            logging.error(f"[CUT_VIDEO] Error cutting video: {e}", exc_info=True)
            return None
    ...
    def loop_video_to_duration(self, video_path: str, target_duration: float) -> str:
        """Loop a video to meet a target duration.
        ...
        """
        try:
            logging.info(f"[LOOP_VIDEO] Starting video loop operation")
            logging.info(f"[LOOP_VIDEO] Target duration: {target_duration:.2f}s")
            
            # Load the original video
            clip = VideoFileClip(video_path)
            original_duration = clip.duration
            
            logging.info(f"[LOOP_VIDEO] Original video duration: {original_duration:.2f}s")
            
            # Calculate how many times we need to loop
            loops_needed = int(target_duration / original_duration) + 1
            logging.info(f"[LOOP_VIDEO] Loops needed: {loops_needed}")
            ...
            final_clip = looped_clip.subclip(0, target_duration)
            
            # Save the looped video
            unique_id = uuid.uuid4()
            assets_dir = os.path.join(self.base_dir, '..', 'assets')
            os.makedirs(assets_dir, exist_ok=True)
            output_path = os.path.join(assets_dir, f"looped_video_{unique_id}.mp4")
            
            logging.info(f"[LOOP_VIDEO] Writing looped video to: {output_path}")
            ...
            logging.info(f"[LOOP_VIDEO] ✓ Video looped successfully: {output_path}")
            return output_path
            
        except Exception as e:
            logging.error(f"[LOOP_VIDEO] ✗ Error looping video: {e}", exc_info=True)
            return None
